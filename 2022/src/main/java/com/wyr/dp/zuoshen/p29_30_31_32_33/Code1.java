...
public class Code1 {
    ...
    //8.1练习
    public int process81(char[] chs, int L, int R){
        if(L==R){
            return 1;
        }
        if(L+1==R){
        ...
        //最长回文子序列不以L和R结尾
        int p1=process81(chs,L+1,R-1);
        int p2=process81(chs,L,R-1);
        int p3=process81(chs,L+1,R);
        //最长回文子序列以L和R结尾
        int p4= chs[L]==chs[R]?2+process81(chs,L+1,R-1):0;
        return Math.max(Math.max(Math.max(p1,p2),p3),p4);
    }
